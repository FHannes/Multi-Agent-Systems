...
public final class PointUtil {
  ...
  public static Optional<Point> intersectionPoint(Point own1, Point own2,
  ...
  public static double angle(Connection<?> connection) {
    return angle(connection.from(), connection.to());
  }

  public static Point perp(Connection<?> conn, double distOnLine, double distFromLine) {
    return PointUtil.perp(conn.from(), conn.to(), distOnLine, distFromLine);
  }

  public static Point perp(Point from, Point to, double distOnLine, double distFromLine) {
  ...
  public static Point on(Connection<?> conn, double dist) {
    return PointUtil.on(conn.from(), conn.to(), dist);
  }

  public static Point on(Point from, Point to, double dist) {
  ...
  public static Point unit(Point from, Point to) {
    return normalize(Point.diff(from, to));
  }

  public static Point unit(Connection<?> conn) {
    return unit(conn.from(), conn.to());
  }

  public static double length(Point from, Point to) {
    return Point.distance(from, to);
  }

  public static double length(Connection<?> conn) {
  ...
  public static Point sub(Point point, double value) {
    return new Point(point.x - value, point.y - value);
  }

  public static Point add(Point point, double value) {
    return new Point(point.x + value, point.y + value);
  }
}
